...
test('width drag tracks the shell 1:1 with transitions disabled (issue #92)', async ({ page }) => {
  ...
  // The seeded session opens the right panel by default.
  const panel = page.locator('[data-bs-panel="right"]')
  await expect(panel).toBeAttached({ timeout: 90_000 })
  const strip = page.locator('[data-bs-resize-strip="width"]')
  await expect(strip).toBeAttached({ timeout: 90_000 })
  ...
  await page.evaluate(() => {
    ...
    const strip = document.querySelector('[data-bs-resize-strip="width"]')
  ...
  expect(box, 'the width strip must be visible for the drag').not.toBeNull()
  const startX = box!.x + box!.width / 2
  const startY = box!.y + Math.min(120, box!.height / 2 + 60)
