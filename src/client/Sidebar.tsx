...
import {
  ...
  resizeFloat, resizeSplitIn, setBottomHeight, setDockSide, setFloatPos, setFloatSize, setWidth,
  toggleBottomPanel, toggleExpanded, toggleFloating, togglePanel,
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  /** Horizontal delta of a width resize. Right-dock drags the panel's LEFT
   *  edge, so growing it means moving the pointer left (startX - x); left-dock
   *  drags its RIGHT edge, so the sign inverts (fork addition). */
  const widthDelta = (startX: number, clientX: number): number =>
    state?.dockSide === 'left' ? clientX - startX : startX - clientX
  ...
  const applyDrag = (width: number, height: number): void => {
    ...
    if (state?.dockSide === 'left') {
      // Left-dock mirror: the whole shell shifts right with the panel, so the
      // bottom panel's LEFT edge follows the panel's right edge.
      bottomRef.current?.style.setProperty('left', `${(centerRectRef.current.left - (state?.width ?? 0)) + width}px`)
    } else {
  ...
  const abortDrag = (reset: () => void, event?: { clientX: number; clientY: number }): void => {
    ...
    if (pending !== null) {
      ...
      if (draggingWidth) {
        width = clampWidth(widthDrag.current.startWidth + widthDelta(widthDrag.current.startX, event.clientX))
        ...
        width = clampWidth(cornerDrag.current.startWidth + widthDelta(cornerDrag.current.startX, event.clientX))
  ...
  useEffect(() => {
    // Floating detaches the workbench from the viewport edges, so it must not
    // push the app shell at all (fork addition).
    const floating = snapshot.state?.floating === true
    const { width, height } = layoutPushSize({
      narrow,
      panelOpen: !floating && snapshot.state?.panelOpen === true,
      bottomOpen: !floating && snapshot.state?.bottomOpen === true,
      width: snapshot.state?.width ?? 0,
      bottomHeight: snapshot.state?.bottomHeight ?? 0,
      viewportWidth: viewport.width,
      viewportHeight: layoutViewportHeight,
    })
    const bottomPush = !floating && !narrow && snapshot.state?.bottomOpen === true
      ? height + keyboardInset
      : 0
    writeGeometry(width, bottomPush)
  }, [narrow, snapshot.state?.floating, snapshot.state?.panelOpen, snapshot.state?.width, snapshot.state?.bottomOpen, snapshot.state?.bottomHeight, viewport.width, layoutViewportHeight, keyboardInset])
          ...
          {!narrow && (
            <div
              ...
              onPointerMove={(event) => {
                ...
                const width = clampWidth(startWidth + widthDelta(startX, event.clientX))
